...
def main(args_eval, resume_preempt=False):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(device)
        train_acc,train_loss_epochs,_ = run_one_epoch(
            device=device,
            training=True,
            ...
            use_bfloat16=use_bfloat16)
        # print(f'printing train_loss epochs: {train_loss_epochs}')
        # print(f'printing train_loss length epochs: {len(train_loss_epochs)}')

        val_acc, _, val_acc_epochs = run_one_epoch(
            device=device,
            training=False,
            ...
            use_bfloat16=use_bfloat16)
        # print(f'printing val acc epochs: {val_acc_epochs}')
        # print(f'printing val acc epochs LENGTH: {len(val_acc_epochs)}')

        logger.info('[%5d] train: %.3f%% test: %.3f%%' % (epoch + 1, train_acc, val_acc))
        
        all_losses.append(train_loss_epochs)
        all_accuracies.append(val_acc_epochs)
        
        if rank == 0:
            csv_logger.log(epoch + 1, train_acc, val_acc)
        save_checkpoint(epoch + 1)

    # # Move losses and accuracies to CPU and convert to NumPy arrays
    # all_losses_cpu = [loss.item() for loss in all_losses]
    # all_accuracies_cpu = [acc.item() for acc in all_accuracies]
    print(len(all_accuracies))
    print(all_accuracies)
    print(type(all_accuracies))
    print(all_accuracies[0])
    print(type(all_accuracies[0]))
    
    
    print(len(all_losses))
    print(all_losses)
    print(type(all_losses))
    print(all_losses[0])
    print(type(all_losses[0]))
    ...
    plt.tight_layout()
    plt.savefig('loss_acc_phase_4_ATTEND_30_epochs.jpg')
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
            ...
            # labels = torch.tensor(data[1]).to(device)  # Convert data[1] to a tensor before moving it to the device
        ...
        # loss = loss + l2_reg_strength * l2_reg_loss
        ...
        if itr % 20 == 0:
            ...
            accuracies.append(top1_meter.avg.item())
            print(accuracies)
    avg_loss = epoch_loss / num_batches
    avg_accuracy = epoch_accuracy / num_batches
    return top1_meter.avg, avg_loss, avg_accuracy
